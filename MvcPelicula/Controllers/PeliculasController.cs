...
namespace MvcPelicula.Controllers
{
    ...
    {
        private MvcPeliculaContext db = new MvcPeliculaContext();

        // GET: Peliculas
        public ActionResult Index(string buscarString, string generoPelicula, string buscarnString)

        {
            var peliculasn = from l in db.Peliculas
                            select l;
            ...
            if (!String.IsNullOrEmpty(buscarnString))
            {
                peliculas = peliculas.Where(y => y.Precio.Contains(buscarnString));
            }
